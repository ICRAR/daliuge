...
class HelmClient:
    ...
    def _set_physical_graph(self, physical_graph_content):
        ...
        self._num_islands, self._num_nodes = _find_resources(self._physical_graph_file)
    ...
    def launch_helm(self):
        ...
        if self._submit:
            ...
            instruction = (
                f"helm install {self._deploy_name} {self._chart_name}/  "
                f"--values {self._chart_name}{os.sep}custom-values.yaml"
            )
            print(subprocess.check_output([instruction], shell=True).decode("utf-8"))
            query = str(
                subprocess.check_output(["kubectl get svc -o wide"], shell=True)
            )
