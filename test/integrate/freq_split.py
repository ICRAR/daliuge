...
if __name__ == "__main__":
    ...
    parser.add_option("-i", "--input_ms", action="store", type="string",
                      dest="infile", help="Input measurementset (Required)")
    parser.add_option("-o", "--output_dir", action="store", type="string",
                      dest="outdir", help="Output directory (Required)")
    parser.add_option("-k", "--work_dir", action="store", type="string",
                      dest="work_dir", help="Working directory (Required)")
    parser.add_option("-c", "--casa_bin_dir", action="store", type="string",
                      dest="casa_bin_dir", help="CASA executable directory (Required)")

    # optional mstransform parameters
    parser.add_option("-n", "--min_freq", action="store", type="int",
                      dest="min_freq", default=1020, help="Mininum frquency (MHz)")
    parser.add_option("-x", "--max_freq", action="store", type="int",
                      dest="max_freq", default=1028, help="Maximum frquency (MHz)")
    parser.add_option("-s", "--freq_step", action="store", type="float",
                      dest="step_freq", default=4, help="Subband width(MHz)")
    parser.add_option("-w", "--freq_width", action="store", type="float",
                      dest="width_freq", default=15.625, help="Frquency resolution(KHz)")
    ...
    do_split(options.infile, options.outdir, options.min_freq,
             options.max_freq, options.step_freq, options.width_freq,
             options.work_dir, options.casa_bin_dir)
