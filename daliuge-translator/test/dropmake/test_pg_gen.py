...
class TestPGPartition(unittest.TestCase):
    ...
    def test_metis_pgtp_gen_pg(self):
        ...
        for lg_name in self.partitionMethodLGs:
            ...
            result = pgtp.result()
            self.assertIsNone(
                result["total_data_movement"], f"Incorrect partitioning for: {lg_name}"
            )
            ...
            result = pgtp.result()
            self.assertEqual(
                total_data_movement_pgspec[lg_name],
    ...
    def test_metis_pgtp_gen_pg_island(self):
        ...
        for lg_name in self.partitionMethodLGs:
            ...
            self.assertTrue(
                "num_islands" in pgtp.result(), f"No islands in PG spec for: {lg_name}"
            )
            self.assertEqual(
                2,
                pgtp.result()["num_islands"],
    ...
    def test_mysarkar_pgtp_gen_pg(self):
        ...
        for lg_name in self.partitionMethodLGs:
            ...
            self.assertGreater(
                pre_spec_result["exec_time"],
                pgtp.result()["exec_time"],
    ...
    def test_mysarkar_pgtp_gen_pg_island(self):
        ...
        for lg_name in self.partitionMethodLGs:
            ...
            if lg_name != "cont_img_mvp.graph":
                ...
                self.assertEqual(
                    self.SARKAR_PARTITION_RESULTS_GEN_ISLAND[lg_name], partition_results
                )
            else:
                pgtp.merge_partitions(len(node_list) - nb_islands, form_island=False)
                pgtp.to_pg_spec(node_list, num_islands=nb_islands)
                self.assertEqual(
                    self.SARKAR_PARTITION_RESULTS_GEN_ISLAND[lg_name],
                    pgtp.result(),
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            "You have run the dataset generator for this test suite.\n"
            "\n"
