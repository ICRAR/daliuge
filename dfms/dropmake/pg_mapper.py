...
2. The complexity of maintaining te DoP constraint within each partition is too
   ...
   The key issue here is that we tried to use graph
   theory (antichains) to prevent something bad from hapening in the future. This
   in turn requires significant computation (e.g. complete transitive closures).
   Instead, we could just "go" to the future, disallow bad things via simulations
   by playing out the HEFT algorithm
...
popular METIS algorithm --- cannot minimise completion time
...
class ResourceCapability(object):
    ...
    def bandwidth(self, nodeA, nodeB):
        ...
        return intra_inter_ratio if gap == 0 else 1

    def comp(self, drop, comp_node):
        """
        Comput cost in time
        """
        return drop['tw']

    def comm(self, drop_edge_weight, nodeA, nodeB):
        """
        Communication cost in time
        """
        return drop_edge_weight / float(self.bandwidth(nodeA, nodeB))
